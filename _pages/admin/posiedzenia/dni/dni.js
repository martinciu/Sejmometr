var Dzien = Class.create({
  initialize: function(data){
    ...
    this.btnSprawdzStatusyWypowiedzi =  mBrowser.itemDiv.down('.btnSprawdzStatusyWypowiedzi').observe('click', function(){
      $S('statusy_wystapien', this.id, function(data){
        alert( 'count: '+data.length );
      });
    }.bind(this));
    this.btnSprawdzStatusyGlosowan = mBrowser.itemDiv.down('.btnSprawdzStatusyGlosowan').observe('click', function(){
      $S('statusy_glosowan', this.id, function(data){
        alert( 'count: '+data.length );
      });
    }.bind(this));
  }
...
var Dzien = Class.create({
  initialize: function(data){
    ...
    $('ostatnie_punkty_a').observe('click', function(){
  ...
  save: function(){
    if( mBrowser.enabled ) {
      ...
      params.id = this.id;
	    ...
	    $S('punkty_wypowiedzi/zapisz', params, this.onSave.bind(this), function(){
	      mBrowser.disable_loading
	      this.btnSave.enable();
	    }.bind(this));
