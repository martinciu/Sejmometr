...
class Scribd {
   ...
   */
    ...
    $r = $this->postRequest("docs.getList", $params);
    return array_values( $r['resultset'] );    
	}
   ...
   */
	public function delete($doc_id){
		...
		if( is_array($_SESSION['_scribd_list']) ) foreach( $_SESSION['_scribd_list'] as $i=>$item ) {
		  if( $item['doc_id']==$doc_id ) { unset( $_SESSION[$i] ); }
		}
		return $result;		
	}
	  /**
	...
	private function postRequest($method, $params){
		...
		curl_close($ch);
			...
			if($result['stat'] == 'fail'){
				...
				if($error_array['code']!='507') { throw new Exception($error_array['message'], $error_array['code']); }
